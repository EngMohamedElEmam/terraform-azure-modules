...
resource "azurerm_public_ip" "pip" {
  ...
  name                = lower("${var.virtual_machine_name}-${var.location}-0${count.index + 1}-ip")
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = var.public_ip_allocation_method
  sku                 = var.public_ip_sku
  sku_tier            = var.public_ip_sku_tier
  domain_name_label   = var.instances_count != 1 ? element(concat(var.domain_name_label, [""]), count.index) : null #var.domain_name_label
  zones               = var.public_ip_availability_zone

  lifecycle {
    ignore_changes = [
      tags, domain_name_label
    ]
  }
...
resource "azurerm_network_security_rule" "nsg_rule" {
  ...
  destination_address_prefix  = element(concat(var.nsg_subnet_address_prefix, [""]), 0)
